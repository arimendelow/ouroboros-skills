...
- **Move migration.** `migrations/01-move-to-ourostack-desk.md` runs through `desk:session-start-migrations` and moves only V2 installs.
  - Claude Code: it runs when `desk@ouroboros-skills` is installed from a marketplace that tracks `v2-alpha` and `desk@ourostack` is not yet installed. It adds the `ourostack/desk` marketplace with automatic updates on and copies the saved desk binding to `plugins/data/desk-ourostack/`, never overwriting an existing one. It then reinstalls Crew, Superpowers, Plain Language and, last, Desk from `@ourostack` in their original scopes and project directories, and uninstalls the old copies while keeping their data.
  - V1 plugins: it never uninstalls an old plugin that a plugin staying in `ouroboros-skills` depends on. It reads those dependencies from each staying plugin's installed manifest and keeps every moved plugin if a manifest cannot be read. Work Suite therefore keeps its `plain-language@ouroboros-skills`. The old marketplace is removed only when nothing from it remains; otherwise the report names the V1 plugins that stay, says that V2 replaces V1, and gives the exact commands to remove them.
  - Failures: a failed uninstall stops the migration with the command to run.
  - Agency: it rewrites only `github:ourostack/ouroboros-skills:plugins/<desk|superpowers|plain-language|crew>@v2-alpha` to `github:ourostack/desk:plugins/<name>@main`. `@main` and no-ref (V1) coordinates stay. It writes a `.pre-ourostack-desk` backup once, never touches an existing `agency.toml.bak`, and keeps a symlinked `agency.toml` a symlink. Only a Claude Code move needs `jq`.
